...
# ----------------------------------------------------------------------------------------------------------------------
#
#      Class handling stjohnsrandom dataset.
...
class stjohnsrandomDataset(PointCloudDataset):
    """Class to handle stjohns dataset."""

    def __init__(self, config, set='training', use_potentials=True, load_data=True):
        """
        This dataset is small enough to be stored in-memory, so load all point clouds here
        """
        PointCloudDataset.__init__(self, 'stjohnsrandom')
        ...
        #self.path = '/mnt/SN750/00_Donnees_SSD/02_CCCOT/st_johns_lidar/st_johns_random'
        #self.path = '/space/partner/nrcan/geobase/work/transfer/work/deep_learning/lidar/ConvPoint_St-John_2020'
        self.path = '/KPConv-PyTorch/raw_random'
        ...
        '''
        ply_path = Folder to files created after prepare_stjohns_ply() function 
        ...
        self.cloud_names = [x.split('.')[0] for x in listdir(join(self.path, self.original_las_path))]
        ...
        self.test_splits = self.all_splits
        ...
        if self.set == 'training':
            ...
            raise ValueError('Unknown set for stjohnsrandom data: ', self.set)
        ...
        for i, f in enumerate(self.cloud_names):
            if self.set == 'training':
                ...
                raise ValueError('Unknown set for stjohns data: ', self.set)
...
class stjohnsrandomSampler(Sampler):
    """Sampler for stjohns"""

    def __init__(self, dataset: stjohnsrandomDataset):
...
class stjohnsrandomustomBatch:
    """Custom batch definition with memory pinning for stjohns"""
...
def stjohnsrandomCollate(batch_data):
    return stjohnsrandomustomBatch(batch_data)
